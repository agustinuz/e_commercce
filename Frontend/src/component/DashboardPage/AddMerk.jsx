...
const AddMerk = ({ fetchData }) => {
  ...
  const [merkNameToDelete, setMerkNameToDelete] = useState("");
  const [merkNameToUpdate, setMerkNameToUpdate] = useState("");
  ...
  const fetchMerk = async () => {
    try {
      const response = await axios.get("http://localhost:5000/getMerk");
  ...
  const handleCreateMerk = async () => {
    try {
      const response = await axios.post("http://localhost:5000/createMerk", {
        nameMerk: newMerk,
      });
      console.log("Response from creating merk:", response.data);
      fetchMerk();
      setNewMerk("");
      setShowModal(false);
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete("http://localhost:5000/deleteMerk", {
        data: { nameMerk: merkNameToDelete },
      });
      setShowDeleteModal(false);
      // Lakukan hal lain yang diperlukan setelah penghapusan
      fetchMerk(); // Ambil ulang merk setelah penghapusan
    } catch (error) {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put("http://localhost:5000/updateMerk", {
        nameMerk: merkNameToUpdate,
        newNameMerk: newName,
      });
      setShowUpdateModal(false);
      // Lakukan hal lain yang diperlukan setelah pembaruan
      fetchMerk(); // Ambil ulang merk setelah pembaruan
    } catch (error) {
      console.error("Error:", error);
    }
  };
  ...
  const handleNextPage = () => {
    if (currentPage < Math.ceil(merk.length / itemsPerPage)) {
      setCurrentPage(currentPage + 1);
    }
  };
  ...
  const currentMerk = merk.slice(indexOfFirstItem, indexOfLastItem);

  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(merk.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
        ...
        <div id="layoutSidenav_content">
          <div className="container-fluid px-4">
            ...
            <figcaption className="blockquote-footer mb-5">
              all for <cite title="Source Title">Brand</cite>
            </figcaption>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
              <Modal.Header closeButton>
                <Modal.Title>Tambah Merk</Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <input
                  type="text"
                  ...
                  placeholder="Enter new merk name"
                  className="form-control"
                />
              </Modal.Body>
              <Modal.Footer>
                ...
                <Button variant="primary" onClick={handleCreateMerk}>
                  Create Merk
                </Button>
              </Modal.Footer>
            </Modal>
                        ...
                        <td>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setMerkNameToDelete(merk.nameMerk);
                            }}
                          >
                            Delete
                          </Button>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setMerkNameToUpdate(merk.nameMerk);
                            }}
                          >
                            Update
                          </Button>
                          ...
                          >
                            <Modal.Header closeButton>
                              <Modal.Title>Update Merk</Modal.Title>
                            </Modal.Header>
                            <Modal.Body>
                              <Form.Group controlId="formMerkNameToUpdate">
                                <Form.Label>Merk Name</Form.Label>
                                <Form.Control
                                  type="text"
                                  value={merkNameToUpdate}
                                  onChange={(e) =>
                                    setMerkNameToUpdate(e.target.value)
                                  }
                                />
                              </Form.Group>
